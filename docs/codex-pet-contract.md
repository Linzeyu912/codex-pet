...
结构校验之外还必须运行 `pnpm assets:continuity` 并查看逐行动画预览。连续动作需要保持统一角色比例、稳定脚底或下腹锚点、平滑的相邻帧质心变化和接近的首尾循环帧。不要在同一行拼接来自不同动作或不同角度的独立姿势；`idle` 只做轻微呼吸与眨眼，row 7 `running` 表示原地工作/处理而非字面奔跑，侧躺可作为 row 5 `failed` 的连续反应，但不得突然混入背面或翻滚姿势。
